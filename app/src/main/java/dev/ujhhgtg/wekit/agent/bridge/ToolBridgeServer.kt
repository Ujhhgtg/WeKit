...
class ToolBridgeServer(
    ...
    private val audit: (ToolBridgeSession.AuditEntry) -> Unit = {},
) {
    ...
    fun open(owner: String, visibility: ToolVisibility, context: CoroutineContext): ToolBridgeSession {
        val token = ByteArray(ToolBridgeProtocol.TOKEN_BYTES).also(SecureRandom()::nextBytes)
            .joinToString("") { "%02x".format(it) }
        return ToolBridgeSession(registry, executor, visibility, context, token, owner, audit).also { tokens[token] = it }
    }
    ...
    suspend fun <T> withOneShot(
        ...
        val session = open(owner, visibility, context)
    ...
    private fun serve(socket: java.net.Socket) {
        socket.use {
            ...
            val response = if (session == null) "{\"ok\":false,\"error\":\"unauthorized\"}" else
                kotlinx.coroutines.runBlocking { session.handle(frame.payload) }
            ToolBridgeProtocol.write(it.getOutputStream(), frame.token, response)
        }
    }

    companion object { const val MAX_CONNECTIONS = 16 }
}
