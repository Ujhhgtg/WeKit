...
static int find_r_debug_callback(dl_phdr_info *info, size_t /*size*/,
                                 void *data) {
  auto *finder = static_cast<DebugFinder *>(data);
  if (!info || !info->dlpi_phdr || finder->debug)
    return 1;
  ...
  for (ElfW(Half) i = 0; i < info->dlpi_phnum; ++i) {
    ...
    if (phdr.p_type != PT_DYNAMIC)
      continue;
    const auto *dynamic =
        reinterpret_cast<const ElfW(Dyn) *>(info->dlpi_addr + phdr.p_vaddr);
...
int so_hide_path(const char *needle) {
  ...
  // the same path; older vendor kernels simply retain their original maps.
  {
    ...
    if (probe < 0) {
      LOGI("so_hide_path: memfd_create unavailable; hidden %d linker names",
           linker_hidden);
