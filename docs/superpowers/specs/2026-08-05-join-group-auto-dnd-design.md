...
功能覆盖通过邀请、扫码以及其他最终汇入同一群成员同步路径的“自己加入已有群聊”方式。不处理当前用户自己创建或拉起群聊的初始创建操作；如果当前用户之后退出该群并重新加入，则属于新的入群行为，应自动免打扰。
...
- 当前用户自己创建或拉起群聊的初始创建操作不触发；该创建路径由 `createchatroom` 成功响应直接调用群存储 helper，不经过本功能 Hook 的通用 ModContact 群成员同步入口。
- 如果当前用户之后退出自己创建的群并重新加入，则属于新的入群行为，满足成员状态转换时应触发；不要求追踪永久历史创建来源。
...
3. 自己创建或拉起群的初始创建操作不自动免打扰；退出后重新加入则自动免打扰；
...
- 独立持久化历史状态；
- 对不同入群来源提供不同配置；
- 不对当前用户自己创建或拉起群聊的初始创建操作触发自动免打扰；退出并重新加入则按新的入群行为处理。
