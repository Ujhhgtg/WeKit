...
**Goal:** Detect when the current user joins another person’s WeChat group through the shared room-member synchronization path, then submit one native group Do Not Disturb operation without affecting self-created groups or ordinary member updates.

**Architecture:** Add a feature-local DexKit hook for WeChat’s `ChatroomMembersLogic.N(...)` synchronization method. The hook snapshots a lightweight WeKit-owned chatroom state before and after the original method, accepts only a confirmed `self absent -> self present` transition with a non-self owner, and dispatches one asynchronous `WeConversationApi.setDnd(roomId, true)` call. Keep database access in `WeDatabaseApi`; do not use `WeDatabaseListenerApi` as the trigger and do not persist processed room IDs.
...
- Only mute groups the current user joins; never mute groups the current user creates or pulls up.
...
- `app/src/main/java/dev/ujhhgtg/wekit/features/api/core/models/WeChatroomSyncState.kt` — immutable WeKit-owned snapshot containing room ID, normalized member IDs, owner, and optional member version.
- `app/src/main/java/dev/ujhhgtg/wekit/features/items/contacts/AutoDndAfterJoinGroup.kt` — switch feature, DexKit resolver, before/after hook, owner exclusion, async DND dispatch, bounded in-memory deduplication.
...
- `app/src/main/java/dev/ujhhgtg/wekit/features/api/core/WeDatabaseApi.kt` — add a bound-SQL helper that reads one chatroom’s raw member list, owner, and version into `WeChatroomSyncState?` without resolving contacts.
...
## Task 1: Verify cross-version sync signatures and owner exclusion
...
- [ ] **Step 2: Trace the create-room and self-join paths.**
  ...
  Follow the callers and inspect the room-service/create code and contact-sync assembler. Confirm which argument or persisted field identifies a self-created/pulled-up room, and whether `roomOwner == WeApi.selfWxId` is sufficient in each supported version.
...
- [ ] **Step 5: Finish the source verification record.**
  ...
  Capture the confirmed per-version method signatures, owner exclusion evidence, and resolver constraints in the implementation task notes or code-review description. Do not add decompiled-source copies to the repository.
...
**Interfaces:**
- Produces `WeChatroomSyncState(roomId: String, memberIds: Set<String>, roomOwner: String, memberVersion: Int?)` (use the repository’s established nullable/version type if source inspection proves a different type).
...
- [ ] **Step 2: Add the immutable state model.**
  ...
  Define only the fields required by the feature: room ID, normalized member-ID set, owner, and nullable member version. Do not retain host model objects, contacts, display names, or full member records.

- [ ] **Step 3: Add raw chatroom-state SQL.**

  Query the `chatroom` row by `chatroomname`, selecting `memberlist`, `roomowner`, and the version column confirmed in Task 1. Return `null` for no row or unavailable database; preserve an empty member set when the row exists with an empty list.
...
- Produces a decision function with this exact signature, used unchanged by Task 4:
  `shouldMuteJoinedGroup(oldState: WeChatroomSyncState?, newState: WeChatroomSyncState, selfWxId: String, syncOwner: String?): Boolean`.
...
- [ ] **Step 1: Write failing pure tests for accepted transitions.**
  ...
  - no old row + new member set contains self + owner is another user -> `true`;
  - old member set excludes self + new set contains self + owner is another user -> `true`;
  ...
  - self is sync owner -> `false`;
  - persisted self owner -> `false`;
...
- [ ] **Step 3: Implement the minimal pure predicate.**
  ...
  Require a known new member set containing self. Treat missing/unknown state as no action. Require owner evidence that the room is not self-created; fail closed when the new-room owner is unavailable. Keep ordinary existing-room updates from triggering unless they establish the exact self transition.
...
- [ ] **Step 3: Implement the before-hook snapshot.**
  ...
  Extract the verified room ID and sync owner from the arguments, reject unsupported suffixes, and read the old state before the original method. Associate the snapshot with the current hook invocation using the hook parameter identity/lifecycle confirmed from the project hook API; never use one global mutable snapshot.

- [ ] **Step 4: Implement the after-hook transition.**

  Retrieve and remove the matching old snapshot, read the post-sync state, use the Task 3 predicate, and return without action for missing state, empty/unconfirmed member data, self-owned rooms, existing self membership, or unsupported room IDs.
...
- [ ] **Step 4: Document manual device validation.**
  ...
  Execute the spec’s scenarios on real WeChat: external invitation/QR join, self-created-group exclusion, empty-then-complete member sync, other-member changes, both room suffixes, disabled state, restart/replay, already-muted state, and one failed DND submission. Record outcomes and logs; desktop tests do not replace this validation.
